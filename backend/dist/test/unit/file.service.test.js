...
describe('File Service', () => {
    ...
    describe('uploadFile', () => {
        ...
        it('should throw error for unsupported file type', async () => {
            ...
            await expect(fileService.uploadFile(invalidFileData, 1)).rejects.toThrow(new ApiError(httpStatus.UNSUPPORTED_MEDIA_TYPE, 'Unsupported file type'));
        });
        it('should throw error for file size exceeding limit', async () => {
            ...
            await expect(fileService.uploadFile(largeFileData, 1)).rejects.toThrow(new ApiError(httpStatus.REQUEST_ENTITY_TOO_LARGE, 'File size exceeds 10MB limit'));
        });
        it('should handle storage upload failure', async () => {
            mockStorage.uploadData.mockRejectedValue(new Error('Storage error'));
            await expect(fileService.uploadFile(mockFileData, 1)).rejects.toThrow(new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Failed to upload file'));
        });
    });
    ...
    describe('updateFileById', () => {
        ...
        it('should throw error if file not found', async () => {
            mockPrisma.file.findFirst.mockResolvedValue(null);
            await expect(fileService.updateFileById(1, 1, newFileData)).rejects.toThrow(new ApiError(httpStatus.NOT_FOUND, 'File not found'));
        });
        it('should throw error for unsupported file type', async () => {
            mockPrisma.file.findFirst.mockResolvedValue(existingFile);
            const invalidFileData = {
            ...
            await expect(fileService.updateFileById(1, 1, invalidFileData)).rejects.toThrow(new ApiError(httpStatus.UNSUPPORTED_MEDIA_TYPE, 'Unsupported file type'));
        });
    });
    describe('deleteFileById', () => {
        ...
        it('should throw error if file not found', async () => {
            mockPrisma.file.findFirst.mockResolvedValue(null);
            await expect(fileService.deleteFileById(1, 1)).rejects.toThrow(new ApiError(httpStatus.NOT_FOUND, 'File not found'));
        });
        it('should handle storage deletion failure', async () => {
            mockPrisma.file.findFirst.mockResolvedValue(mockFile);
            mockStorage.deleteFile.mockRejectedValue(new Error('Storage error'));
            await expect(fileService.deleteFileById(1, 1)).rejects.toThrow(new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Failed to delete file'));
        });
    });
});
